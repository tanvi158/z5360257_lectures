...
def get_tics(pth):
    ...
    with open(pth, 'r') as file:
        ...
        for line in file:
            ...
            if line:
                # Split the line at '=' and strip quotes
                parts = [part.strip('"') for part in line.split('=')]

                # Ensure we have exactly 2 parts and they are not empty
                if len(parts) == 2 and all(parts):
                    exchange, ticker = parts

                    # Format the strings to conform with the rules
...
def read_dat(tic):
    ...
    # Construct the file path by joining the directory and the file name
    file_path = os.path.join(DATDIR, f"{tic}_prc.dat")

    # Make a list to store the lines
    lines = []
...
def line_to_dict(line):
    ...
    # Iterate over the column widths and the column names together
    for col_name, width in zip(COLUMNS, COLWIDTHS):
        ...
        # Insert the extracted value into the result dictionary
        result[col_name] = value

        # Move the starting index to the next position
        start += width
...
def verify_tickers(tic_exchange_dic, tickers_lst=None):
    ...
    """
    # If tickers_lst is provided, check if it's not empty
    if tickers_lst is not None:
        if len(tickers_lst) == 0:
            raise Exception("tickers_lst should not be an empty list.")

        # Check if each ticker in tickers_lst is a key in tic_exchange_dic
...
def verify_cols(col_lst=None):
    ...
    if not col_lst:
        raise Exception("Empty list of columns provided.")

    for col in col_lst:
        if col not in COLUMNS:
            raise Exception(f"Column '{col}' is not recognized.")
...
def create_data_dict(tic_exchange_dic, tickers_lst=None, col_lst=None):
    ...
    """
    # If tickers_lst is None, use all tickers from tic_exchange_dic
    if tickers_lst is None:
        tickers_lst = list(tic_exchange_dic.keys())

    # If col_lst is None, use all predefined columns (assuming COLUMNS is a global constant)
    if col_lst is None:
        col_lst = COLUMNS  # assuming COLUMNS is defined globally or you can pass it as an argument
    ...
    # Process each ticker
    for ticker in tickers_lst:
        # Read data lines for the ticker
        data_lines = read_dat(ticker)
        ...
        for line in data_lines:
            ...
            # Filter out only the keys from col_lst
            filtered_dict = {key: line_dict[key] for key in col_lst if key in line_dict}

            # Add this to the ticker data
            ticker_data.append(filtered_dict)

        # Add ticker data to the result dictionary
        result_dic[ticker] = {
...
def create_json(data_dict, pth):
    ...
    """
    try:
        with open(pth, 'w', encoding='utf-8') as json_file:
            json.dump(data_dict, json_file, ensure_ascii=False, indent=4)
    except IOError as e:
        print(f"An error occurred while writing to {pth}: {e}")
